...
> 3. **空格鍵 / 再一次 (Space Bar / More)**：延續當前模式與人設，產出一組全新的、1000 字符的極致提示詞。
...
### Phase 2: 選定創意主題
由 AI 根據亞文化 (Alt-fashion/Geek/BDSM) 構思具備故事張力的場景。

### Phase 3: 單組極致產出 & 快捷連讀
- 輸出 Image Prompt 與 Motion & Audio 於單一 Code Block。
- **模式覆寫 (Mode Override)**：在任何階段輸入 [1] 或 [2]，AI 必須立即重置當前模式，主動選取主題並產出該模式的提示詞。
- **模式切換與換人**：在任何階段輸入 [1]、[2] 或 [3]，AI 必須立即執行對應動作並產出提示詞。
- **後續觸發**：提示「[1] 真人 | [2] 3D | [3] 換人設 | [Space] 變體」。
