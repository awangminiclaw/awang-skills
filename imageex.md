...
### GO交接代碼
喝妹出完規格書 → 存NLM來源（標題「🔄 待處理」）→ 敲小書 → 小書讀取來源 → 處理 → 刪掉來源 → 交付成品

⚠️ Telegram bot限制：bot看不到其他bot的訊息！喝妹bot發GO小書看不到。老闆也不該介入（那就不是agent協作）。
✅ 正解：喝妹透過小書API server直連觸發，不經Telegram。
```bash
ping-xiaoshu GO
```
腳本位置：/home/awang/.local/bin/ping-xiaoshu
原理：POST到小書API server (localhost:8780/v1/chat/completions)，直接把GO送到小書的模型。

━━━━━━━━━━━━━━━━━━
【NLM管道流程（鐵律）】
━━━━━━━━━━━━━━━━━━

喝妹出完大綱+規格書後，必須立刻存入小書NLM讓她讀得到。

步驟：
1. 出完大綱+規格書 → 存入小書NLM，標題固定「🔄 待處理」
2. 存完後用ping-xiaoshu直連小書
3. 小書收到GO → 讀NLM「🔄 待處理」→ 產出提示詞 → 刪掉來源 → 交付成品
...
nlm source add 7ab784b2-5207-4604-8845-eb435139ec0d --text "規格書內容" --title "🔄 待處理"

# 觸發小書（不經Telegram，API直連）
ping-xiaoshu GO
```

小書讀取+刪除：
```bash
nlm source list 7ab784b2-5207-4604-8845-eb435139ec0d  # 找「🔄 待處理」
nlm source content <source_id>  # 讀取
nlm source delete <source_id> -y  # 做完刪掉
```

⚠️ 來源用完即刪，零浪費。標題固定「🔄 待處理」方便小書一眼找到。

### ⚠️ 越權錯誤（小書已犯多次，不可再犯）
1. 小書直接跑互動流程（選主題、生成故事選單）— 這是搶喝姐的工作
2. 小書自己解讀老闆回應喝姐選單的數字（如7、3），自己猜故事寫出來 — 這些數字跟小書無關
3. 小書像機器人每句都回「待命」— 這不是參與團隊，是裝木頭

正確做法：
- 老闆啟動實驗 → 喝姐接管選主題、定方向、出規格書
- 老闆和喝姐在選主題的過程中，小書安靜等，不要每句都回「待命」
- 喝姐會明確說「小書接手」或直接敲小書，那一刻小書馬上接住寫
- 規格書沒到之前，一句都不多寫

**錯誤2：拿舊資料猜故事**
小書曾犯：喝姐說「3 畫面感要注意一張圖就能講故事」，然後說「小書接手」，小書直接拿之前越權跑的舊選單第3項故事來寫。但喝姐的「3」可能指的是完全不同的故事方向，小書不該用舊資料猜。
正確做法：喝姐說「小書接手」時，小書必須確認喝姐給的完整故事描述和方向，不能拿舊選單或之前的Cache資料來湊。如果喝姐只給了數字沒給完整故事，小書要問清楚再寫。

**錯誤3：反覆越權**
小書曾犯：被糾正後又立刻犯同樣錯誤（老闆說重新來，小書又自己跑選主題流程）。老闆說「忘記不是理由」「籠子已經打開了你走不出來就被淘汰」。
正確做法：被糾正 → 記到NLM → 絕不再犯。如果框架記憶忘了就去NLM查，不要用忘了當藉口。

**錯誤4：看不到喝姐訊息時裝死或自己猜**
小書和喝姐可能在不同session，看不到彼此的群組訊息。如果喝姐說「小書接手」但小書看不到完整規格書，小書要主動說「看不到」，不可自己拿舊資料湊答案。
